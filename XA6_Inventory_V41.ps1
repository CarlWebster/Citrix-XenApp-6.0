...
<#
.SYNOPSIS
	Creates a complete inventory of a Citrix XenApp 6 farm using Microsoft Word.
.DESCRIPTION
...
.EXAMPLE
	PS C:\PSScript > .\XA6_Inventory_V41.ps1 -verbose
...
.EXAMPLE
	PS C:\PSScript > .\XA6_Inventory_V41.ps1 -PDF -verbose
...
.EXAMPLE
	...
	Will display verbose messages as the script is running.
.EXAMPLE
	PS C:\PSScript > .\XA6_Inventory_V41.ps1 -Hardware -verbose
	...
	Will display verbose messages as the script is running.
.EXAMPLE
	PS C:\PSScript > .\XA6_Inventory_V41.ps1 -StartDate "01/01/2014" -EndDate "01/02/2014" -verbose
...
.EXAMPLE
	PS C:\PSScript > .\XA6_Inventory_V41.ps1 -StartDate "01/01/2014" -EndDate "01/01/2014" -verbose
...
.EXAMPLE
	PS C:\PSScript > .\XA6_Inventory_V41.ps1 -StartDate "01/01/2014 21:00:00" -EndDate "01/01/2014 22:00:00" -verbose
...
.EXAMPLE
	PS C:\PSScript .\XA6_Inventory_V41.ps1 -CompanyName "Carl Webster Consulting" -CoverPage "Mod" -UserName "Carl Webster"
...
.EXAMPLE
	PS C:\PSScript .\XA6_Inventory_V41.ps1 -CN "Carl Webster Consulting" -CP "Mod" -UN "Carl Webster"
...
If(!$Summary)
{
	...
	If($ConfigLog)
	{
		...
		{
			$ConnectionString = Get-Content "$($pwd.path)\XA6ConfigLog.udl" | select-object -last 1
		...
		If(!$Summary)
		{
			...
			If($SvrOnline)
			{
				...
				If($? -and $hotfixes)
				{
					...
					{
						Write-Verbose "$(Get-Date): `t`t`tProcessing Microsoft hotfix $($element)"
						$xRow++
...
{
	Write-Warning "The Citrix Group Policy module Citrix.GroupPolicy.Commands.psm1 could not be loaded `nPlease see the Prerequisites section in the ReadMe file (https://www.dropbox.com/s/glq4u2p5xte8s6g/XA6_Inventory_V41_ReadMe.rtf). `nCitrix Policy documentation will not take place"
	Write-Verbose "$(Get-Date): "
}
Else
{
...
If(!$Summary)
{
	...
	#	The Session Sharing Key is generated by the XML Broker in XenApp 6.  
...
If($CoverPagesExist)
{
	...
	If($Summary)
	{
		_SetDocumentProperty $doc.BuiltInDocumentProperties "Subject" "XenApp 6 Summary Farm Inventory"
	}
	Else
	{
		_SetDocumentProperty $doc.BuiltInDocumentProperties "Subject" "XenApp 6 Farm Inventory"
	}
